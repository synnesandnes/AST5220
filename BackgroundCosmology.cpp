...
BackgroundCosmology::BackgroundCosmology(
  ...
  Neff(Neff)
    ...
    void BackgroundCosmology::solve(){
        ...
        Vector x_array = Utils::linspace(x_sstart, x_end, npts);
        ...
        // Creating the spline
        eta_of_x_spline.create(x_array, eta_aray, "eta_of_x");
        Utils::EndTiming("Eta");
...
double BackgroundCosmology::derivatives_Hdx(double x) const{
    double a = exp(x);
...
// Not sure if either: Will always return zero as we are computing without curvature

// Or if I should use formula for Omega_K as defined in report

// But as we are not asked to calculate OmegaK i will comment it out for now
